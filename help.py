...
general_select = SelectMenu(
    custom_id="help_command_select",
    placeholder="選擇一個指令",
    options=[SelectOption(label=i, value=i) for i in ["help", "info", "whopinged", "feedback"]],
    min_values=1,
    max_values=1,
)
general_embed = raweb(
...
nsfw_embed = raweb(
    "NSFW指令", "這裡是可以色色的指令的說明。\n你可以使用下面的選單來瀏覽幫助頁面。\n\n如果你想提出建議或是回報問題，請使用 `/feedback` 指令。"
)
admin_select = SelectMenu(
    custom_id="help_command_select",
    placeholder="選擇一個指令",
    options=[
        SelectOption(label=i, value=i)
...
invite = Button(
    ...
    label="邀請我",
    url="https://discord.com/api/oauth2/authorize?client_id=1030626357669011557&permissions=8&scope=bot%20applications.commands",
)
...
class help(Extension):
    ...
    async def help(self, ctx: CommandContext):
        ...
        await ctx.send(
            embeds=maineb, components=[_select(ctx.channel.nsfw), invite], ephemeral=True
        )

    @extension_component("help_goback")
    async def _help_goback(self, ctx: ComponentContext):
        await ctx.defer(edit_origin=True)
        await ctx.edit(embeds=maineb, components=[_select(ctx.channel.nsfw), invite])
    ...
    async def _help_select(self, ctx: ComponentContext, selected=None):
        ...
        await ctx.edit(embeds=eb, components=[menu, invite, goback])
    ...
    async def _help_command_select(self, ctx: ComponentContext, selected=None):
        ...
        match selected[0]:
            case "help":
                eb = _command("help", "顯示幫助頁面。", "/help", "/help")
            case "info":
                eb = _command(
                    "info", "顯示各種資訊。", "/info <子指令> <參數>", f"/info user <@{self.client.me.id}>"
                )
            case "whopinged":
                eb = _command("whopinged", "找出過去12小時內最後在這個伺服器@你的人。", "/whopinged", "/whopinged")
            case "feedback":
                eb = _command("feedback", "回報問題或是提出建議。", "/feedback", "/feedback")
            case "image":
                eb = _command("image", "隨機發送各種圖片 (safe for work)。", "/image <圖片類別>", "/image cat")
            case "reaction":
                eb = _command("reaction", "以圖片做出回應。", "/reaction <回應類別>", "/reaction sleepy")
            case "generate":
                eb = _command(
                    "generate", "隨機生成各種圖片。", "/generate <生成類別> <參數>", "/generate qr hello"
                )
            case "twitter":
                eb = _command(
                    "twitter", "搜尋Twitter", "/twitter <子指令> <搜尋>", "/twitter user @itsrqtl"
                )
            case "typing":
                eb = _command("typing", "讓我不斷輸入中...", "/typing <子指令>", "/typing start")
            case "say":
                eb = _command("say", "讓我說話。", "/say <內容>", "/say hello")
            case "8ball":
                eb = _command("8ball", "問問神奇8號球的答案？", "/8ball <問題>", "/8ball 今天天氣好嗎？")
            case "google-tutorial":
                eb = _command(
                    "google-tutorial",
                    "讓我教你如何使用Google。",
                    "/google-tutorial [語言]",
            ...
            case "lengthen":
                eb = _command(
                    "lengthen", "幫你把網址變長。", "/lengthen <網址>", "/lengthen https://google.com"
                )
            case "lmgtfy":
                eb = _command("lmgtfy", "不會google嗎...", "/lmgtfy <搜尋>", "/lmgtfy how to use google")
            case "translate":
                eb = _command(
                    "translate", "翻譯文字。", "/translate <內容> <語言> [原始語言]", "/translate hello zh-hant"
                )
            case "random":
                eb = _command("random", "隨機選擇一個數字。", "/random <min> <max>", "/random 1 100")
            case "unknown":
                eb = _command("unknown", "一個野生的指令出現了！", "/unknown", "/unknown")
            case "whatanime":
                eb = _command("whatanime", "搜尋截圖來源。", "/whatanime <圖片>")
