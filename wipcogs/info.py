...
class general(Extension):
    ...
    async def channel(self, ctx: CommandContext, channel: Channel = None):
        ...
        await ctx.send(embeds=Embed(description=channel.mention, fields=[
            ...
            EmbedField(name="創建時間", value=channel.id.timestamp.strftime("%Y-%m-%d %H:%M"), inline=True),  
    ...
    async def server_info(self, ctx: CommandContext):
        ...
        await ctx.send(
            embeds=Embed(fields=[
                ...
                EmbedField(name=":calendar_spiral: 創建時間", value=f'<t:{round(ctx.guild.id.timestamp.timestamp())}:R>', inline=True),
                EmbedField(name=":star: 擁有者", value=owner.mention, inline=True),
                EmbedField(name=":speech_balloon: 頻道數", value=f"文字頻道: {text}\n語音頻道: {voice}\n分類: {category}", inline=True),
                EmbedField(name=":busts_in_silhouette: 成員", value=f"成員數: {ctx.guild.member_count}\n身份組: {len(ctx.guild.roles)}", inline=True),
                EmbedField(name=":arrow_up: 加成", value=f"數量: {ctx.guild.premium_subscription_count if ctx.guild.premium_subscription_count else 0}\n等級: {ctx.guild.premium_tier if ctx.guild.premium_tier else 0}", inline=True),
                EmbedField(name=":laughing: 表情符號", value=f"{len(ctx.guild.emojis)}/{'50'if not ctx.guild.premium_tier or ctx.guild.premium_tier == 0 else '100' if ctx.guild.premium_tier == 1 else '150' if ctx.guild.premium_tier == 2 else '250'}", inline=True),
            ],author=EmbedAuthor(name=ctx.guild.name, icon_url=ctx.guild.icon_url), color=randint(0, 0xFFFFFF))
            )
    ...
    async def bot_status(self, ctx: CommandContext):
        ...
        embed = Embed(title="狀態", description="你那麼想知道我的狀態... 嗎？", fields=[
            EmbedField(name="📒 版本", value=ver, inline=False),
            EmbedField(name="📊 數據", value=f"伺服器: **{len(self.client.guilds)}**\n頻道: **{channels}**", inline=False),
            EmbedField(name="📈 狀態", value=f"RAM 使用量: **{round(tracemalloc.get_traced_memory()[0]/1000000, 2)}**mb\n延遲: 大概 **{abs(round(self.client.latency, 2))}**ms 吧？但是我會一直陪著你喔！", inline=False),
        ], color=randint(0, 0xFFFFFF))
