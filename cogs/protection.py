...
class protect(Extension):
    ...
    async def on_message_create(self, message: Message):
        ...
        if channel.type == ChannelType.DM:
            return
        pinged = []
        if message.mention_everyone and (Permissions.MENTION_EVERYONE in permissions or message.author.id == guild.owner_id):
        ...
        if message.mentions:
            for i in message.mentions:
                if (not 'bot' in i or i['bot'] == False) and int(i['id']) not in pinged and i['id'] != str(message.author.id):
                    pinged.append(int(i['id']))
        for i in pinged:
            await self._ping.replace_one({"user_id": i, "guild_id": int(guild.id)}, {"user_id": int(i), "guild_id": int(guild.id), "author": int(message.author.id), "create": datetime.utcnow()}, upsert=True) 

    @extension_command()
    async def whopinged(self, ctx: CommandContext):
        """尋找12小時內最後在這個伺服器@你的人"""
        await ctx.defer(ephemeral=True)
        doc = await self._ping.find_one({"user_id": int(ctx.author.id), "guild_id": int(ctx.guild_id)}, {"_id": 0, "author": 1, "create": 1})
        if not doc:
            return await ctx.send(":x: baka 最近12小時沒有人在這個伺服器@過你 (@role 暫不適用)")
        await ctx.send(embeds=raweb("找到了！", f"最後一次在這個伺服器@你的人是 <@{doc['author']}>\nUTC時間是 {doc['create'].strftime('%Y-%m-%d %H:%M:%S')}"))
    ...
    async def on_message_delete(self, message: Message):
        ...
        if message.mentions:
            for i in message.mentions:
                if (not 'bot' in i or i['bot'] == False) and i['id'] != str(message.author.id):
        ...
        if everyone:
            ...
            if len(victims) > 0:
                ...
                content += "\n".join(victims)
        channel = await message.get_channel()
        await channel.send(embeds=raweb(f"抓到 Ghost Ping 了！", desc=f"{message.author.mention} 這樣可不行喔！\n以下的受害者被Ghost ping了...\n\n{content}"))
